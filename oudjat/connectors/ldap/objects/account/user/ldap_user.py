...
class LDAPUser(LDAPAccount):
    ...
    def __init__(self, ldap_entry: "LDAPEntry", **kwargs: Any) -> None:
        ...
        if ms_acc_ctl is not None:
            self._enabled: bool = not LDAPAccountCtlFlag.is_disabled(ms_acc_ctl)
            self._pwd_expires: bool = LDAPAccountCtlFlag.pwd_expires(ms_acc_ctl)
            self._pwd_expired: bool = LDAPAccountCtlFlag.pwd_expired(ms_acc_ctl)
            self._pwd_required: bool = LDAPAccountCtlFlag.pwd_required(ms_acc_ctl)
            self._is_locked: bool = LDAPAccountCtlFlag.is_locked(ms_acc_ctl)

            self._ldap_obj_flags.update(LDAPAccountCtlFlag.flags(ms_acc_ctl))
